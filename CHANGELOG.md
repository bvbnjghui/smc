...
重構了 smc-analyzer.js，將市場結構事件明確區分為「趨勢延續 (BOS)」與「趨勢轉變 (CHoCH)」，使分析邏輯更精確、更貼近 SMC 實戰原則。

BOS (Break of Structure) 現在用於標示順應主要趨勢的結構突破，代表趨勢的健康延續。

CHoCH (Change of Character) 現在專門用於標示在流動性掠奪後，首次出現的、預示趨勢可能反轉的結構突破。

相應地更新了側邊欄 UI、圖表繪製邏輯及回測引擎，以完全整合新的市場結構定義。

新增 (Added)
專業化回測引擎:

動態倉位規模計算: 回測引擎不再使用固定的風險乘數，而是採用專業的風險管理模型，根據「帳戶權益」、「風險百分比」及「止損距離」動態計算每筆交易的倉位大小。

整合 Top-Down 分析邏輯: 在回測設定中新增「HTF 偏好 (Bias)」選項（僅做多／僅做空／多空皆可）。模擬器將嚴格過濾，只執行符合使用者設定的高時間週期方向的交易。

市場特性與風險調整: 在側邊欄新增「市場特性調整」區塊，允許使用者設定「波動率調整 (SL 乘數)」，動態放寬或收緊止損距離，以適應不同市場（如股票 vs. 加密貨幣）的波動性。

變更 (Changed)
使用者說明文件:

根據最新的分析邏輯與實戰流程，全面更新了「如何解讀圖表」說明文件。新的指南從「核心交易流程 (Top-Down)」出發，提供系統化的操作步驟，而不僅僅是術語解釋。
