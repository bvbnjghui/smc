...
新增 (Added)
參數優化系統: 實現了完整的參數優化功能，包括批量測試、敏感度分析和智能建議。

修正 (Fixed)
參數優化相關修復:
- 修復參數敏感度分析的計算錯誤問題，增強了相關係數計算的錯誤處理
- 修復參數優化UI卡住問題，添加了適當的UI更新機制
- 修復參數優化進度顯示問題，改進了進度更新頻率和顯示
- 修復參數優化阻塞UI的問題，使用異步處理避免UI凍結
- 修復優化建議顯示參數名稱的問題，使用用戶友好的參數名稱
- 修復隨機測試進度顯示總數不正確的問題，修復了總數計算邏輯
- 修復 LightweightCharts 初始化錯誤問題，修復了全域引用問題
- 修復參數優化全選功能問題，實現了參數組和參數的雙向同步
- 修復 actualTotalTests 變數初始化順序錯誤問題，重新組織了變數宣告順序
- 修復設定過期K棒數參數的小數點顯示問題，確保整數參數正確顯示為整數
- 移除 HTML 模板中的 x-init 和 x-effect 調試訊息

改進 (Improved)
參數優化建議系統: 降低建議閾值並增加解釋，包含弱相關參數並提供更實用的建議。

清理 (Cleaned)
程式碼清理: 移除所有調試用的 console.log 訊息，提升程式碼品質。

[3.2.1] - 2025-08-15

修正 (Fixed)
回測引擎穩定性: 修正了在將回測引擎模組化拆分後，因 candles (K棒數據) 參數未能正確傳遞到 signal-finder.js 模組，而導致在執行回測時發生 Cannot read properties of undefined (reading 'find') 錯誤並造成程式崩潰的問題。

[3.2.0] - 2025-08-14

重構 (Refactored)
前端架構: 將 main.js 進行模組化拆分，把所有狀態管理邏輯獨立到一個新的 smc/modules/state-manager.js 模組中，大幅提升了程式碼的可讀性與可維護性。

修正 (Fixed)
設定保存邏輯: 修正了因 hasOwnProperty 方法與 Alpine.js 的響應式代理 (Proxy) 不兼容，而導致設定被保存為空物件的問題。

初始載入設定: 修正了應用程式在首次載入時，圖表分析未能正確應用已保存設定的時序問題。

參數遺漏: 修正了在模組化拆分後，intervals 參數遺失導致側邊欄無法渲染的問題。

[3.1.0] - 2025-08-14

新增 (Added)
進場權重策略 (Confluence Scoring System):

為「區域反應型」進場策略引入了專業的權重計分系統，旨在過濾低品質信號並提高勝率。

使用者現在可以在回測設定中，自訂一個「進場權重門檻」，並為不同的 confluent (匯集) 信號分配不同的權重。

修正 (Fixed)
回測引擎邏輯修正:

修正了「區域反應型」策略因「前視偏誤 (Look-ahead Bias)」而導致勝率異常高達 100% 的問題。

修正了「區域反應型」策略中因未處理「K 棒內風險」而導致虧損交易被遺漏的問題。

Killzone 過濾器: 恢復了在 backtester.js 中被意外移除的 Killzone 過濾邏輯。

[3.0.2] - 2025-08-13

修正 (Fixed)
初始載入邏輯: 修正了應用程式初始化時的時序問題，確保從 localStorage 讀取的設定能夠在首次載入圖表數據時被正確應用。

指標分析範圍: 修正了「僅分析可見範圍」功能會錯誤地限制全域指標計算的問題。現在 EMA 與 ATR 將始終在完整的數據集上進行計算。

載入動畫: 統一了載入動畫的體驗，在圖表上顯示一致的載入疊層。

新增 (Added)
使用者引導: 為側邊欄中的 EMA、MTA 及 ATR 等進階功能設定，新增了詳細的說明 Tooltip。
